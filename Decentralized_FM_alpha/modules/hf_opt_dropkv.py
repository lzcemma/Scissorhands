...
class GPTBlock(OPTDecoderLayer):
    ...
    def from_pretrained(cls, model_path, config=None, layer_index=None):
        ...
        module.self_attn.compress_flag = False
        if layer_index >= 5 and layer_index <= 35:
            module.self_attn.compress_flag = True
            module.self_attn.start_compress_length = 100
            module.self_attn.lru_window = int(os.environ.get("LRU_WINDOW", None))
            module.self_attn.position_window = int(
                os.environ.get("POSITION_WINDOW", None)
            )
            module.self_attn.keep_ratio = []
            module.self_attn.lru_keep_ratio = []
            print(
                f"position_window: {module.self_attn.position_window}, lru_window: {module.self_attn.lru_window}"
            )
