...
class DummyRequestProcessor:
    ...
    def set_arguments(self, args):
        ...
        if (
            args.echo_prompt
            and args.input_seq_length == self.tokenizer.model_max_length + 1
            and args.generate_seq_length == 0
        ):
    ...
    def get_dataloader(self, batch_size, num_workers=0):
        ...
        dataset = JsonDataset(
            "Hello, my dog is",
            # ['fruit options: apples, bananas. Vehicle options: car, truck']*1000,
            self.tokenizer,
    ...
    def add_result(self, inputs, outputs, batch_time=None):
        ...
        for i in range(batch_size):
            ...
            print(
                f">>>>>> batch_time: {batch_time:.4f}s, batch_size: {batch_size} <<<<<<"
            )
            item = {
                "choices": [],
                "request_time": {
            ...
            for i_ret, output_dict in enumerate(outputs):
                choice = {
                    "text": (
                        tokenizer.decode(output_dict["token_ids"][i][n_pads:])
                    ...
                    "logprobs": {
                        "tokens": (
                            tokenizer.convert_ids_to_tokens(
                                output_dict["token_ids"][i][n_pads:]
                                if "token_ids" in output_dict
                                else []
                            )
                        ),
                        "token_logprobs": (
                            output_dict["token_logprobs"][i][n_pads:].tolist()
                            if "token_logprobs" in output_dict
                            else []
                        ),
                        "top_logprobs": (
                            [
                                {
                                    ...
                                    for topk_id, top_logprob in zip(
                                        topk_ids, top_logprobs
                                    )
                                }
...
class RequestProcessor:
    def __init__(self, request_path, tokenizer):
        ...
        if basename.endswith("jsonl"):
            ...
            with open(self.request_path) as f:
                self.data = [{"request": line} for line in json.load(f)]
        else:
    ...
    def set_arguments(self, args):
        ...
        if hasattr(args, "overwrite_request_args") and args.overwrite_request_args:
            ...
            if args.model_type != "t5":
                ...
                self.tokenizer.model_max_length = 1000  # avoid padding
                ...
                args.batch_size = (
                    max(
                        budget
                        // (
                            (args.input_seq_length + args.generate_seq_length)
                            * self.num_completions
                        ),
                        args.token_micro_batch_size,
                    )
                    // args.token_micro_batch_size
                    * args.token_micro_batch_size
                )
                args.batch_size = min(
                    args.batch_size, 64
                )  # TODO: if batch size is too large, the comm will stuck.
                ...
                args.batch_size = max(
                    budget // (args.input_seq_length + args.generate_seq_length), 1
                )
        ...
        if (
            args.echo_prompt
            and max_input_seq_length == self.tokenizer.model_max_length + 1
            and args.generate_seq_length == 0
        ):
            # special case! to support 2049 tokens
            args.input_seq_length = self.tokenizer.model_max_length + 1
